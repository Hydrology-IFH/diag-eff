...
def import_camels_obs_sim(path, sep=r"\s+"):
    ...
    obs_sim.drop(['HR', 'SWE', 'PRCP', 'RAIM', 'TAIR', 'PET', 'ET'], axis=1,
                 inplace=True)
...
def fdc_obs_sim_ax(obs, sim, ax, fig_num):
    ...
    prob_obs = [(ranks_obs[i]/(len(obs['obs'])+1)) for i in
                range(len(obs['obs']))]

    ranks_sim = sp.stats.rankdata(sim['sim'], method='ordinal')
    ranks_sim = ranks_sim[::-1]
    prob_sim = [(ranks_sim[i]/(len(sim['sim'])+1)) for i in
                range(len(sim['sim']))]

    ax.plot(prob_obs, obs['obs'], lw=2, color='blue', alpha=.5,
            label='Observed')
    ax.plot(prob_sim, sim['sim'], lw=2, ls='-.', color='red',
            label='Manipulated')
